...
const AgentDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	...
	if (device === 'mobile') {
	return (
		<Stack className="agent-detail-page" component="section">
			<Stack className="container">
			{/* Agent header */}
			<Stack className="agent-info" component="header">
				<img
				className="avatar"
				src={agent?.memberImage ? `${REACT_APP_API_URL}/${agent?.memberImage}` : '/img/profile/defaultUser.svg'}
				alt={agent?.memberFullName ?? agent?.memberNick ?? 'Agent profile'}
				loading="lazy"
				/>
				<Box
				component="div"
				...
				onKeyDown={(e: React.KeyboardEvent) => {
					if (e.key === 'Enter' || e.key === ' ') redirectToMemberPageHandler(agent?._id as string);
				}}
				aria-label="View agent profile"
				>
				<strong className="name">{agent?.memberFullName ?? agent?.memberNick}</strong>
				<div className="contact">
					<img src="/img/icons/call.svg" alt="" aria-hidden="true" />
					<span className="phone">{agent?.memberPhone}</span>
				</div>
				</Box>
			</Stack>
			...
			{/* Reviews */}
			<Stack className="review-box" component="section" aria-label="Reviews">
