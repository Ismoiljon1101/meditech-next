...
import InstrumentCard from '../../libs/components/instrument/InstrumentCard';
import useDeviceDetect from '../../libs/hooks/useDeviceDetect';
import withLayoutBasic from '../../libs/components/layout/LayoutBasic';
import Filter from '../../libs/components/instrument/Filter';
...
const EquipmentList: NextPage = ({ initialInput, ...props }: any) => {
	...
	const [searchFilter, setSearchFilter] = useState<InstrumentsInquiry>(
	...
	const {
		...
		onCompleted: (data: T) => {
			setInstruments(data?.getInstruments?.list);
			setTotal(data?.getInstruments?.metaCounter[0]?.total);
		},
	});
	...
	const sortingHandler = (e: React.MouseEvent<HTMLLIElement>) => {
		switch (e.currentTarget.id) {
			case 'new':
				...
				break;
			case 'lowest':
				setSearchFilter({ ...searchFilter, sort: 'instrumentPrice', direction: Direction.ASC });
				setFilterSortName('Lowest Price');
				break;
			case 'highest':
				setSearchFilter({ ...searchFilter, sort: 'instrumentPrice', direction: Direction.DESC });
							...
							<Stack className="pagination-config">
								...
								{instruments.length !== 0 && (
									<Stack className="total-result">
										<Typography>
											Total {total} instrument{total > 1 ? 's' : ''} available
										</Typography>
									</Stack>
								)}
							</Stack>
