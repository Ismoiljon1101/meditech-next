...
import { InstrumentBrand, InstrumentLocation, InstrumentType } from '../../enums/instrument.enum';
import { REACT_APP_API_URL, instrumentSize } from '../../config';
import { InstrumentInput } from '../../types/instrument/instrument.input';
import axios from 'axios';
import { getJwtToken } from '../../auth';
import { sweetErrorHandling, sweetMixinErrorAlert, sweetMixinSuccessAlert } from '../../sweetAlert';
import { useMutation, useQuery, useReactiveVar } from '@apollo/client';
import { userVar } from '../../../apollo/store';
import { CREATE_INSTRUMENT, UPDATE_INSTRUMENT } from '../../../apollo/user/mutation';
import { GET_INSTRUMENT } from '../../../apollo/user/query';

const AddNewInstrument = ({ initialValues, ...props }: any) => {
	const device = useDeviceDetect();
	const router = useRouter();
	const inputRef = useRef<any>(null);
	const [insertInstrumentData, setInsertInstrumentData] = useState<InstrumentInput>(initialValues);
	...
	const [instrumentBrands] = useState<InstrumentBrand[]>(Object.values(InstrumentBrand));
	...
	const {
		...
		variables: { input: router.query.instrumentId },
	});

	/** LIFECYCLES **/
	useEffect(() => {
		const ins = getInstrumentData?.getInstrument;
		setInsertInstrumentData({
			...insertInstrumentData,
			instrumentTitle: ins ? ins.instrumentTitle : '',
			instrumentPrice: ins ? ins.instrumentPrice : 0,
			instrumentType: ins ? ins.instrumentType : '',
			instrumentLocation: ins ? ins.instrumentLocation : '',
			instrumentBrand: ins ? ins.instrumentBrand : '',
			instrumentAddress: ins ? ins.instrumentAddress : '',
			instrumentRent: ins ? ins.instrumentRent : false,
			instrumentCondition: ins ? ins.instrumentCondition : 0,
			instrumentQuantity: ins ? ins.instrumentQuantity : 0,
			instrumentSize: ins ? ins.instrumentSize : 0,
			instrumentDesc: ins ? ins.instrumentDesc : '',
			instrumentImages: ins ? ins.instrumentImages : [],
		});
	}, [getInstrumentLoading, getInstrumentData]);
	...
	async function uploadImages() {
		try {
			...
			setInsertInstrumentData({ ...insertInstrumentData, instrumentImages: responseImages });
		} catch (err: any) {
	...
	const doDisabledCheck = () => {
		if (
			insertInstrumentData.instrumentTitle === '' ||
			insertInstrumentData.instrumentPrice === 0 || // @ts-ignore
			insertInstrumentData.instrumentType === '' || // @ts-ignore
			insertInstrumentData.instrumentLocation === '' || // @ts-ignore
			insertInstrumentData.instrumentAddress === '' ||
			insertInstrumentData.instrumentCondition === 0 ||
			insertInstrumentData.instrumentQuantity === 0 ||
			insertInstrumentData.instrumentSize === 0 ||
			insertInstrumentData.instrumentDesc === '' ||
			insertInstrumentData.instrumentImages.length === 0
		) {
			return true;
		}
	};

	const insertInstrumentHandler = useCallback(async () => {
		try {
			await createInstrument({ variables: { input: insertInstrumentData } });
			await sweetMixinSuccessAlert('Instrument created successfully.');
			await router.push({ pathname: '/mypage', query: { category: 'myInstruments' } });
		} catch (err: any) {
			sweetErrorHandling(err).then();
		}
	}, [insertInstrumentData]);

	const updateInstrumentHandler = useCallback(async () => {
		try {
			// @ts-ignore
			insertInstrumentData._id = getInstrumentData?.getInstrument?._id;
			await updateInstrument({ variables: { input: insertInstrumentData } });
			await sweetMixinSuccessAlert('Instrument updated successfully.');
			await router.push({ pathname: '/mypage', query: { category: 'myInstruments' } });
		} catch (err: any) {
			sweetErrorHandling(err).then();
		}
				...
				<Stack className="config">
					<Stack className="description-box">
						<Stack className="config-column">
							...
							<input
								...
								placeholder={'Title'}
								value={insertInstrumentData.instrumentTitle}
								onChange={({ target: { value } }) =>
									setInsertInstrumentData({ ...insertInstrumentData, instrumentTitle: value })
								}
							/>
						</Stack>
						...
						<Stack className="config-row">
							<Stack className="price-year-after-price">
								...
								<input
									...
									className="description-input"
									placeholder={'Price'}
									value={insertInstrumentData.instrumentPrice}
									onChange={({ target: { value } }) =>
										setInsertInstrumentData({ ...insertInstrumentData, instrumentPrice: parseInt(value) })
									}
								/>
							</Stack>
							<Stack className="price-year-after-price">
								...
								<select
									...
									defaultValue={insertInstrumentData.instrumentType || 'select'}
									value={insertInstrumentData.instrumentType || 'select'}
									onChange={({ target: { value } }) =>
										// @ts-ignore
										setInsertInstrumentData({ ...insertInstrumentData, instrumentType: value })
									}
								>
									<>
						...
						<Stack className="config-row">
							<Stack className="price-year-after-price">
								...
								<select
									className={'select-description'}
									defaultValue={insertInstrumentData.instrumentBrand || 'select'}
									value={insertInstrumentData.instrumentBrand || 'select'}
									onChange={({ target: { value } }) =>
										// @ts-ignore
										setInsertInstrumentData({ ...insertInstrumentData, instrumentBrand: value })
									}
								>
									<>
										<option selected={true} disabled={true} value={'select'}>
										...
										{instrumentBrands.map((brand: any) => (
							...
							<Stack className="price-year-after-price">
								...
								<select
									className={'select-description'}
									defaultValue={insertInstrumentData.instrumentLocation || 'select'}
									value={insertInstrumentData.instrumentLocation || 'select'}
									onChange={({ target: { value } }) =>
										// @ts-ignore
										setInsertInstrumentData({ ...insertInstrumentData, instrumentLocation: value })
									}
								>
									<>
										<option selected={true} disabled={true} value={'select'}>
										...
										{instrumentLocations.map((location: any) => (
						...
						<Stack className="config-row">
							<Stack className="price-year-after-price">
								...
								<input
									...
									className="description-input"
									placeholder={'Address'}
									value={insertInstrumentData.instrumentAddress}
									onChange={({ target: { value } }) =>
										setInsertInstrumentData({ ...insertInstrumentData, instrumentAddress: value })
									}
								/>
							</Stack>
							<Stack className="price-year-after-price">
								<Typography className="title">Lease Available</Typography>
								<select
									className={'select-description'}
									value={insertInstrumentData.instrumentRent ? 'yes' : 'no'}
									defaultValue={insertInstrumentData.instrumentRent ? 'yes' : 'no'}
									onChange={({ target: { value } }) =>
										setInsertInstrumentData({ ...insertInstrumentData, instrumentRent: value === 'yes' })
									}
								>
						...
						<Stack className="config-row">
							<Stack className="price-year-after-price">
								...
								<select
									className={'select-description'}
									value={insertInstrumentData.instrumentCondition || 'select'}
									defaultValue={insertInstrumentData.instrumentCondition || 'select'}
									onChange={({ target: { value } }) =>
										setInsertInstrumentData({ ...insertInstrumentData, instrumentCondition: parseInt(value) })
									}
								>
							...
							<Stack className="price-year-after-price">
								...
								<select
									className={'select-description'}
									value={insertInstrumentData.instrumentQuantity || 'select'}
									defaultValue={insertInstrumentData.instrumentQuantity || 'select'}
									onChange={({ target: { value } }) =>
										setInsertInstrumentData({ ...insertInstrumentData, instrumentQuantity: parseInt(value) })
									}
								>
									<option disabled={true} selected={true} value={'select'}>
										Select
									</option>
									{[1, 2, 3, 4, 5, 10, 20, 50, 100].map((val: number) => (
										<option value={`${val}`} key={val}>
											{val}
										</option>
									))}
							...
							<Stack className="price-year-after-price">
								...
								<select
									className={'select-description'}
									value={insertInstrumentData.instrumentSize || 'select'}
									defaultValue={insertInstrumentData.instrumentSize || 'select'}
									onChange={({ target: { value } }) =>
										setInsertInstrumentData({ ...insertInstrumentData, instrumentSize: parseInt(value) })
									}
								>
									<option disabled={true} selected={true} value={'select'}>
										Select
						...
						<Stack className="config-column">
							...
							<textarea
								...
								value={insertInstrumentData.instrumentDesc}
								onChange={({ target: { value } }) =>
									setInsertInstrumentData({ ...insertInstrumentData, instrumentDesc: value })
								}
							></textarea>
						</Stack>
					</Stack>
					...
					<Stack className="images-box">
						...
						<Stack className="gallery-box">
							{insertInstrumentData?.instrumentImages.map((image: string) => {
					...
					<Stack className="buttons-row">
						{router.query.instrumentId ? (
							<Button className="next-button" disabled={doDisabledCheck()} onClick={updateInstrumentHandler}>
								<Typography className="next-button-text">Save</Typography>
							</Button>
						) : (
							<Button className="next-button" disabled={doDisabledCheck()} onClick={insertInstrumentHandler}>
								<Typography className="next-button-text">Save</Typography>
							</Button>
						)}
					</Stack>
...
AddNewInstrument.defaultProps = {
	initialValues: {
		...
		instrumentBrand: '',
		instrumentAddress: '',
		instrumentRent: false,
		instrumentCondition: 0,
