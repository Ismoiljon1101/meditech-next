...
const RecentlyVisited: NextPage = () => {
	...
	const [recentlyVisited, setRecentlyVisited] = useState<Instrument[]>([]);
			...
			<div id="my-favorites-page">
				...
				<Stack className="favorites-list-box">
					{recentlyVisited?.length ? (
						recentlyVisited?.map((instrument: Instrument) => {
							return <InstrumentCard instrument={instrument} recentlyVisited={true} />;
						})
					) : (
						<div className={'no-data'}>
							<img src="/img/icons/icoAlert.svg" alt="" />
							<p>No Recently Visited Instruments found!</p>
						</div>
					)}
				</Stack>
				{recentlyVisited?.length ? (
					<Stack className="pagination-config">
						...
						<Stack className="total-result">
							<Typography>
								Total {total} recently visited instrument{total > 1 ? 's' : ''}
							</Typography>
						</Stack>
					</Stack>
				) : null}
